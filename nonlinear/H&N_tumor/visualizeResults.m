...
for i = 1 : nFigRow * nFigCol
    ...
    hold on
    plot3(NodeI(FM_indices,1) + deform_label(FM_indices, 1), NodeI(FM_indices, 2) + deform_label(FM_indices, 2), NodeI(FM_indices, 3) + deform_label(FM_indices, 3), 'r*', 'MarkerSize', 12); % FMs in label configuration. 
...
for i = 1 : nFigRow * nFigCol
    ...
    trisurf(FaceI, NodeI(:, 1) + deform_label(:, 1), NodeI(:, 2) + deform_label(:, 2), NodeI(:, 3) + deform_label(:, 3),'FaceColor', 'none', 'EdgeColor', [0.25 0.25 1]); % Label configuration. 
    hold on
    plot3(NodeI(center_indices,1) + deform_label(center_indices, 1), NodeI(center_indices, 2) + deform_label(center_indices, 2), NodeI(center_indices, 3) + deform_label(center_indices, 3), 'r*', 'MarkerSize', 12); % Center points in label configuration.
...
% %% Reference & Label reconstruction
% figure
% for i = 1 : nFigRow * nFigCol
%     
%     deform_label = reshape(test_deformation_label(:, i), 3, [])'; % nNode * 3 matrix. 
...
%     subplot('Position', positionVector);
%     
%     trisurf(FaceI, NodeI(:, 1), NodeI(:, 2), NodeI(:, 3),'FaceColor', 'none', 'EdgeColor', [0.75 0.75 0.15]); % Label configuration. 
%     colormap jet
%     axis equal off
%     
% end
