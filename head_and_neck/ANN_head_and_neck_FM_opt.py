...
def saveLog(lossList_train, lossList_valid, FM_num, PC_num, batch_size, learning_rate, 
    ...
    content = ['FM_num = {}'.format(FM_num),
               'FM_indices (starting from 0) = {}'.format(list(np.sort(center_indices_list[0:FM_num]))),
               'Center_indices_list (starting from 0) = {}'.format(list(center_indices_list)),
...
if __name__ == "__main__":
    ...
    while(True):
        ...
        (neural_net, lossList_train, 
        lossList_valid) = trainValidateNet(train_dataloader, valid_dataloader, 
