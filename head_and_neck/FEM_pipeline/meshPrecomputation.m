...
%% inputs
% read file core mesh
fileID = fopen('core.node');
tLine = str2num(fgetl(fileID));
nNodeI = tLine(1);    
NodeI = zeros(nNodeI, 3);
for i = 1 : nNodeI
    tLine = str2num(fgetl(fileID));
    NodeI(i, :) = tLine(2:end);
end
NodeI = round(NodeI, 6, 'significant');
fclose(fileID);

fileID = fopen('core.face');
tLine = str2num(fgetl(fileID));
nFaceI = tLine(1);
FaceI = zeros(nFaceI, 3);
for i = 1 : nFaceI
    tLine = str2num(fgetl(fileID));
    FaceI(i, :) = tLine(2 : 4) + 1;
end
...
fileID = fopen('core.ele');
tLine = str2num(fgetl(fileID));
nEleI = tLine(1);
EleI = zeros(nEleI, 4);
for i = 1 : nEleI
    tLine = str2num(fgetl(fileID));
    EleI(i, :) = tLine(2 : end) + 1;
end
fclose(fileID);
